      ...
      for instance_number in number_of_iperf_instances_array do
        ...
        if "#{instance_number}" == "0"
          cron "#{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number}" do
            ...
            minute "#$cron_min"
            user 'narunas'
            command "/home/narunas/repos/iperf3-scripts/iperf-server-run.sh #{iperf_server_interfaces_array[interface_number]} #{iperf_server_instances_ports_array[instance_number]}  #{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number} > /tmp/phys-tcp-throughput-test/script-logs/#{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number} 2>&1"
          end
        else
          cron "#{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number}" do
            hour "#$cron_hour"
            minute "#$cron_min"
            user 'narunas'
            command "/home/narunas/repos/iperf3-scripts/iperf-server-run-without-sar.sh #{iperf_server_interfaces_array[interface_number]} #{iperf_server_instances_ports_array[instance_number]}  #{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number} > /tmp/phys-tcp-throughput-test/script-logs/#{iperf_server_interfaces_names_array[interface_number]}_#{iperf_client_interfaces_names_array[interface_number]}_instance-#{instance_number}_mss-#{mss_value}_#{test_number} 2>&1"
           end
        end
      end
